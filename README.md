...
| [`dog/`](dog/) | Python Flask adapter for one robot: hardware transports (WiFi WebSocket / USB serial / mock), behavior framework, ears (whisper), mouth (speaker), eyes (YOLO person detection), mood light, gait learning |
| [`satellite/`](satellite/) | XIAO ESP32S3 Sense sketch: Laika's microphone, camera and RGB mood light, on its own WiFi |
