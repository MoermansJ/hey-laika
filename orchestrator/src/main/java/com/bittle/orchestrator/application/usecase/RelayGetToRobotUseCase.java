...
public class RelayGetToRobotUseCase {

    private final FleetRegistry registry;
    private final RobotAdapterPort adapter;

    public RelayGetToRobotUseCase(FleetRegistry registry, RobotAdapterPort adapter) {
        this.registry = registry;
        this.adapter = adapter;
    }

    public Map<String, Object> execute(String robotId, String path) {
        return adapter.get(registry.get(robotId), path);
    }
}
